	...
	void OnGUI()
	{
		if ( ControllerManagerEnabled && ( m_ControllerManagerState != ControllerManagerState.NONE ) )
		{
			...
			string boxText = ( m_ControllerManagerState == ControllerManagerState.BIND_CONTROLLER_ONE ) ?
							 "Point left controller at base\n and pull trigger." :
							 "Point right controller at base\n and pull trigger.";
			
            GUI.Box(new Rect((7*Screen.width)/32, Screen.height/2, boxWidth, boxHeight), boxText);
            GUI.Box(new Rect((20*Screen.width)/32, Screen.height/2, boxWidth, boxHeight), boxText);
		}
	}
